...
def post_process_data(test_date="06_01_2025", stop_distance=805):
    ...
    for num, cutoff in enumerate(freq_space):
        ...
        for i in range(1, 7):
            ...
            plt.plot(
                accel_df["time"].to_numpy(),
                accel_df["acceleration_y_LOWPASS_filtered (m/s^2)"].to_numpy(),
                label=f"filtered, cutoff={cutoff}",
                color=colors[num],
            )
            plt.xlabel("Time")
            ...
            accel_df.to_csv(
                f"Processed_data/Test_{test_date}/{test_case_string}/Acceleration_data_{cutoff}.csv", index=False
            )
    ...
    plt.plot(
        accel_df["time"].to_numpy(),
        -accel_df["acceleration_y (m/s^2)"].to_numpy(),
        label="Estimated Acceleration (m/s²)",
        color="black",
        alpha=0.2,
        linewidth=0.1,
        zorder=-100,
...
def process_garmin_data(log_number_str, test_date, stop_distance):
    ...
    # Take needed columns
    df_needed = df_OG[
        ["timestamp", "heart_rate", "cadence", "distance", "power", "enhanced_speed", "enhanced_altitude"]
    ]
    ...
    est_acc = df["velocity (m/s)"].diff() / df["time"].diff().dt.total_seconds()
    est_acc = est_acc.shift(-1)
    est_acc.iloc[-1] = est_acc.iloc[-2]
    df["estimated_acceleration (m/s^2)"] = est_acc.fillna(0)
...
def process_wind_speed_data(log_number_str, test_date, start_time, stop_time):
    ...
    # Keep only Time, WindMPS, and TempC; rename columns
    df = df_raw[["Time", "WindMPS", "TempC"]].copy()
    df.columns = ["time", "wind_speed (m/s)", "temperature (C)"]
    ...
    # Group by each second for wind speed and temperature
    df["sec"] = df["time"].dt.floor("S")
    df_grouped = df.groupby("sec").agg({"wind_speed (m/s)": "mean", "temperature (C)": "mean"}).reset_index()
    df_grouped.columns = ["time", "wind_speed (m/s)", "temperature (C)"]

    t0 = start_time.floor("S")
    t1 = stop_time.ceil("S")
    full_seconds = pd.DataFrame({"time": pd.date_range(start=t0, end=t1, freq="1S")})

    # Merge wind speed and temperature onto the full seconds index
    df_1hz = full_seconds.merge(df_grouped, on="time", how="left")

    # Fill missing values for both columns
    df_1hz["wind_speed (m/s)"].ffill(inplace=True)
    df_1hz["wind_speed (m/s)"].bfill(inplace=True)
    df_1hz["temperature (C)"].ffill(inplace=True)
    df_1hz["temperature (C)"].bfill(inplace=True)
...
def combine_upsampled_dataframes(garmin_upsampled_df, wind_sensor_upsampled_df, acceleration_df):
    ...
    combined["temperature (C)"] = w["temperature (C)"]
